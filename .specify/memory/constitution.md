...
- Modified principles:
	...
	- Delivery Workflow and Milestones -> Delivery Workflow and Milestones
...
- Agents are NOT allowed to implement feature/refactor work without a written spec
  ...
  exploratory work.
- Every new spec MUST be created from a newly created feature branch whose name
  matches the spec directory exactly (`specs/<branch-name>/`). Plans, tasks, and
  implementation work for that spec MUST run from that same branch; reusing an
  existing spec directory from a different branch is not allowed.
...
- `/speckit.plan` outputs MUST pass constitution gates before Phase 0 and after
  Phase 1 design.
- `/speckit.specify` outputs MUST create a new uniquely numbered feature branch
  and matching `specs/<branch-name>/` directory, and MUST include offline/privacy,
  manual override, edge-case, and measurable-performance requirements.
...
- Every review of spec/planning workflow changes MUST verify exact branch-to-spec
  mapping remains enforced.
...
**Version**: 1.6.0 | **Ratified**: 2026-03-12 | **Last Amended**: 2026-03-13
